...
export function ManualCorrectionForm({id,company}:{id:string;company:Record<string,unknown>}) {
 ...
 return <form onSubmit={async e=>{e.preventDefault();setM("Zapisuję…");const f=new FormData(e.currentTarget);
  ...
  const manualAll={name:nullable("name"),website:nullable("website"),country:nullable("country"),region:nullable("region"),city:nullable("city"),phone:nullable("phone"),publicEmail:nullable("publicEmail"),nip:nullable("nip"),krs:nullable("krs"),technologies:lines("technologies"),partnershipLevels:lines("partnershipLevels"),serviceDescription:nullable("serviceDescription"),portfolioUrls:lines("portfolioUrls"),reviewCount:reviewRaw===null?null:Number(reviewRaw),reviewSource:nullable("reviewSource"),decisionMakers,publicJobPostings};
  const initial:Record<string,unknown>={name:company.name||null,website:company.website||null,country:company.country||null,region:company.region||null,city:company.city||null,phone:company.phone||null,publicEmail:company.public_email||null,nip:company.nip||null,krs:company.krs||null,technologies:company.technologies||[],partnershipLevels:company.partnership_levels||[],serviceDescription:company.service_description||null,portfolioUrls:company.portfolio_urls||[],reviewCount:company.review_count??null,reviewSource:company.review_source||null,decisionMakers:company.decision_makers||[],publicJobPostings:company.public_job_postings||[]};
  const manual=Object.fromEntries(Object.entries(manualAll).filter(([key,value])=>JSON.stringify(value)!==JSON.stringify(initial[key])));
  if(!Object.keys(manual).length){setM("Brak zmian do zapisania.");return;}
