...
export async function GET(request: Request) {
  ...
  try {
    ...
    const batch = await runBatch(sources.rows.map((x) => new DirectoryConnector({ name: x.name, enabled: x.enabled, url: x.config.url as string | undefined })));
    ...
    for (const company of stale.rows) try { await researchCompany(company.id); rechecked++; } catch (error) {
      recheckErrors++;
