...
export async function POST(request: Request) {
  ...
  try {
    await assertPublicUrl(parsed.data.url); const candidate = (await new ManualUrlConnector(parsed.data.url, parsed.data.name).discover(1))[0];
    const saved = await upsertCandidate(candidate); const result = await researchCompany(saved.id);
